...
// Polyfill for File global (needed for undici compatibility)
...
// Set NODE_OPTIONS to avoid undici issues
...
function areFavorableConditions(conditions, criteria) {
  ...
  let validWindDirections = [];
  if (wind.speed <= 10) {
    // Wind ≤10km/h: any wind direction is OK for good conditions
    validWindDirections = ['N', 'NNE', 'NE', 'ENE', 'E', 'ESE', 'SE', 'SSE', 'S', 'SSW', 'SW', 'WSW', 'W', 'WNW', 'NW', 'NNW'];
    console.log(`Wind speed: ${wind.speed}km/h ≤10km/h, any wind direction OK for good conditions`);
  } else if (wind.speed <= 20) {
    // Wind 10-20km/h: offshore ±3 tolerance for good conditions
    validWindDirections = getOpposingWindDirections(wave.direction, 3);
    console.log(`Wind speed: ${wind.speed}km/h (10-20km/h), using offshore ±3 tolerance for good conditions`);
  } else {
    // Wind >20km/h: too windy for good conditions
    validWindDirections = [];
    console.log(`Wind speed: ${wind.speed}km/h >20km/h, too windy for good conditions`);
  }
  const validPerfectWindDirections = getOpposingWindDirections(wave.direction, 2); // For perfect conditions (offshore ±2)
...
async function sendEmail(conditions) {
  try {
    ...
    const mailOptions = {
      ...
      subject: `🏄‍♂️ Surf Alert: Good conditions at ${process.env.TARGET_BEACH}!`,
      html: `
        <h2>🏄‍♂️ Surf Alert!</h2>
        <p>Good surf conditions detected at <strong>${process.env.TARGET_BEACH}</strong>!</p>
        
        <h3>Good Conditions Found:</h3>
        <pre style="background: #f4f4f4; padding: 15px; border-radius: 5px; font-family: monospace;">${goodConditionsText}</pre>
      ...
      `,
      text: `
        Surf Alert! Good conditions at ${process.env.TARGET_BEACH}!
...
async function main() {
  try {
    ...
    console.log("Environment variables check:", {
      beach: process.env.TARGET_BEACH,
      surfUrl: process.env.SURF_FORECAST_URL,
    ...
    const conditions = await scrapeSurfConditions(process.env.TARGET_BEACH);
